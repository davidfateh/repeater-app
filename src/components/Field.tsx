  ...
  return (
    <div>
      ...
      >
        {!state.selectEnabled && (
          <Paragraph element={'p'}>
            Selected settings: <b>{state.selectedValue}</b>{' '}
            <TextLink onClick={toggleSettingsEnabled}>Change selected</TextLink>
          </Paragraph>
        )}
        ...
        {state.selectEnabled && (
          <div style={{ backgroundColor: '#edf4fc', padding: '15px' }}>
            <Note noteType={'primary'} style={{ marginBottom: '15px' }}>
              Selecting a new settings type from the dropdown will reset your
              fields. Reload the page without saving to revert.
            </Note>
            <FormLabel htmlFor={'TypeDropDown_Select'}>
